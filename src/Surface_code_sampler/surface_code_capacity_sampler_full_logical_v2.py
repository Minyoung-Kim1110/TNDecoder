...
class StimSurfaceBatchSampleFullLogical:
    ...
    batch_x: X-memory experiment used to decode the Z-logical bit.
    batch_z: Z-memory experiment used to decode the X-logical bit.
...
def _qubit_coords(circuit: stim.Circuit) -> Dict[int, Tuple[float, ...]]:
    ...
    for op in circuit:
        ...
        if len(ts) != 1:
            continue
        q = int(ts[0].value)
        out[q] = tuple(float(x) for x in op.gate_args_copy())
    if not out:
        raise RuntimeError("No QUBIT_COORDS metadata found in generated circuit.")
    return out
...
def _infer_logical_support_indices_from_noisy_targets(
    circuit: stim.Circuit,
    noisy_targets,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Infer one representative support for logical X and logical Z directly from
    the noisy-round data-qubit coordinates of Stim's unrotated patch.

    We use:
      - logical X support = left boundary data qubits  (minimum x)
      - logical Z support = bottom boundary data qubits (minimum y)

    Then:
      z_log = parity of Z-components (Y or Z) on logical X support
      x_log = parity of X-components (X or Y) on logical Z support

    This matches the convention residual operator = Z^z_log X^x_log.
    """
    q2c = _qubit_coords(circuit)
    coords = []
    for t in noisy_targets:
        q = int(t.value)
        if q not in q2c:
            raise RuntimeError(f"Qubit {q} missing from QUBIT_COORDS metadata.")
        xy = q2c[q]
        if len(xy) < 2:
            raise RuntimeError(f"Qubit {q} has malformed coordinates {xy}.")
        coords.append((q, float(xy[0]), float(xy[1])))

    xs = np.array([x for _, x, _ in coords], dtype=float)
    ys = np.array([y for _, _, y in coords], dtype=float)
    min_x = float(np.min(xs))
    min_y = float(np.min(ys))
    tol = 1e-9

    support_x = np.array(
        [k for k, (_, x, _) in enumerate(coords) if abs(x - min_x) <= tol],
        dtype=np.int64,
    )
    support_z = np.array(
        [k for k, (_, _, y) in enumerate(coords) if abs(y - min_y) <= tol],
        dtype=np.int64,
    )
    if support_x.size == 0 or support_z.size == 0:
        raise RuntimeError("Failed to infer non-empty logical supports from noisy-round targets.")
    return support_x, support_z


def _logical_bits_from_pauli_pattern(
    pauli_pattern: np.ndarray,
    *,
    logical_x_support_indices: np.ndarray,
    logical_z_support_indices: np.ndarray,
) -> np.ndarray:
    """
    pauli codes:
      0 -> I, 1 -> X, 2 -> Y, 3 -> Z

    z_log is the parity of Z components on a support of logical X:
      codes in {2, 3}

    x_log is the parity of X components on a support of logical Z:
      codes in {1, 2}
    """
    pattern = np.asarray(pauli_pattern, dtype=np.uint8).reshape(-1)

    z_component = np.isin(pattern, np.array([2, 3], dtype=np.uint8))
    x_component = np.isin(pattern, np.array([1, 2], dtype=np.uint8))

    z_log = int(np.sum(z_component[logical_x_support_indices]) % 2)
    x_log = int(np.sum(x_component[logical_z_support_indices]) % 2)
...
def sample_surface_code_capacity_batch_full_logical(
    ...
    *,
    ...
    chosen noisy_round. The same sampled data error is then injected into both
        - surface_code:unrotated_memory_x  (for the X-basis syndrome slice), and
        - surface_code:unrotated_memory_z  (for the Z-basis syndrome slice).

    The returned logical_bits array has columns ordered as (z_log, x_log), computed directly
    from the sampled Pauli pattern on fixed logical-string supports.
    """
    ...
    logical_x_support_indices, logical_z_support_indices = _infer_logical_support_indices_from_noisy_targets(
        marker_x,
        targets_x,
    )
    ...
    for _ in range(shots):
        ...
        logical_bits = _logical_bits_from_pauli_pattern(
            pauli_pattern,
            logical_x_support_indices=logical_x_support_indices,
            logical_z_support_indices=logical_z_support_indices,
        )
        ...
        # Keep the original one-bit observable arrays because the existing PEPS / MWPM
        # basis-specific decoders expect a single observable per batch, but the full
        # logical truth label is computed directly from the sampled Pauli pattern above.
        ...
        logical_bits_list.append(logical_bits)
...
def test_full_logical_nontrivial_truth_labels() -> None:
    data = sample_surface_code_capacity_batch_full_logical(
        distance=3,
        p=0.1,
        shots=200,
    ...
    assert data.logical_bits.shape == (200, 2)
    assert np.any(np.any(data.logical_bits != 0, axis=1)), "truth labels stayed trivial at p=0.1"
...
def run_surface_code_capacity_full_logical_tests() -> None:
    ...
    test_full_logical_nontrivial_truth_labels()
